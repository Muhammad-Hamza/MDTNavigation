...
class MapApplication constructor(
    ...
    private var OFF_ROUTE_TIMER = 15000L
...
//                var index = -1 //findNearestChunk(listOfChunks, lastCurrentLocation!!)
                ...
                /*
                * @index showing the which listOfChunks contain the current Location
                * @nestedIndex show the which point near to the current location
                * */
...
//                })
    ...
    fun findNearestChunk(
        ...
        if (index > 0) {
            ...
            for (j in 0..(index - 1)) {
                Log.e(
                    "FILTER", "${j} = isVisited: " + listOfChunks.get(j).isVisited.toString()
                )
        ...
        Log.e(
            "FILTER", "Nearest chunk found at index: $index and nestedIndex: $nestedIndex"
        )
